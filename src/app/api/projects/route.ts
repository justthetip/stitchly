...
export async function POST(request: Request) {
  try {
    ...
    const projects = await sql`insert into public.projects (owner_id, pattern_id, name, yarn) values (${user.id}, ${body.patternId}, ${body.name.trim()}, ${body.yarn?.trim() || null}) returning id, owner_id, pattern_id, name, status, yarn, current_instruction, started_at, last_worked_at, completed_at` as DbProject[];
